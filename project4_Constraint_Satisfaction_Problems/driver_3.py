...
This is the version I handed in. It is quite "functional": Neither AC_3 or BTS changes their arguments
"""
...
def BTS(sudoku):
    ...
    Test your code on the provided set of puzzles in sudokus_start.txt. Can you solve all puzzles now?
    """
    if is_solved(sudoku):
        return(sudoku)
    ...
    for value in sudoku[tile]: # I could speed up the solution by choosing the values in a clever order.
        current_sudoku = copy.deepcopy(sudoku)  #This copy is not stricly necessary. But I do it anyway, so that BTS does not alter arguments.
...
def gen_board(config):
    sudoku = {}
    for indx, tile in enumerate(TILES):
        number = int(config[indx])
        if number != 0:
...
#sys.argv = ["path", "000000001007050309004800020000000000030005700009420000000003000001000407060278000"]

if len(sys.argv) > 1 :
    sudoku_string = sys.argv[1]
    fh = open('output.txt', 'w')
    fh.write(sudoku_solver(sudoku_string))
    fh.close()
