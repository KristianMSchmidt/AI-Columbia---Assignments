...
This is the next best version. About 43 seconds. Uses heaps.
"""
...
def BTS(sudoku, heap):
    ...
    for value in sudoku[tile]: # I could speed up the solution by choosing order of values according to some heuristic.
        ...
        if result_from_AC3 != False: # in this case AC_3 did not find any problems
            result = BTS(result_from_AC3, heap.copy())
            if result != False:
                return result
...
def gen_constraints():
    ...
    for letter in "ABCDEFGHI":
        for number in "123456789":
            C[letter + number] = set()
            for c in constraints:
                if c[0] == letter+number:
                    C[letter + number].add(c)

    return constraints, C, C2
...
def gen_solve_string(sudoku):
    ...
    return "".join([str(next(iter(val))) for val in sorted(sudoku.values())])
...
def sudoku_solver(sudoku_string):
    ...
    heap = [(len(val), key) for key, val in AC_3_attempt.items()]
    heapq.heapify(heap)

    #BTS_solution = BTS(sudoku)
    BTS_solution = BTS(AC_3_attempt, heap)
...
CONSTRAINTS, C, C2 = gen_constraints()
