...
class Sudoku:
    def __init__(self, sudoku_string):
        self._board = _gen_board(self, sudoku_string)

    def _gen_board(self, sudoku_string):
        """ Generates a sudoku-board from a string """
        board = {}
        for indx, tile in enumerate(TILES):
            number = int(sudoku_string[indx])
            if number != 0:
                board[tile] = set([number])
            else:
                board[tile] = set(range(1,10))

    def _gen_solve_string(self):
        """ Generates solution string from a solved puzzle """
        return "".join([str(next(iter(val))) for _, val in sorted(self._board.items())])

    def _BTS(self, sudoku):
        """
