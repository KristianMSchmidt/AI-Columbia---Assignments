...
from gradient_heuristic41 import heuristic
...
class PlayerAI(BaseAI):
    ...
    def minimax_alpha_beta_DLS(self, grid, depth, players_turn, first_move, start_time, alpha, beta):
        ...
        """
        if time.clock() - start_time >  timeLimit - 0.01:
            print "Time's up!"
    ...
    def minimax_IDDFS(self, grid):
        """
        Minimax. Iterative deepening depth-first-search.
        """
        start_time = time.clock()
        best_move = None
        ...
        for depth in xrange(1000):
            ...
            try:
                best_score, best_move = self.minimax_DLS(grid, depth, True, None, start_time)
                obtained_depth = depth
    ...
    def minimax_DLS(self, grid, depth, players_turn, first_move, start_time):
        """
        ...
        if players_turn: #This is the maximizing player
            ...
            for move in moves:
                ...
                child.move(move)
                if first_move == None:
                    value = self.minimax_DLS(child, depth - 1, False, move, start_time)
                else:
                    value = self.minimax_DLS(child, depth - 1, False, first_move, start_time)
                max_value = max(max_value, value)
            ...
            for cell in cells:
                child = grid.clone()
                child.setCellValue(cell, 2)
                value_2 = self.minimax_DLS(child, depth - 1, True, first_move, start_time)

                child = grid.clone()
                child.setCellValue(cell, 4)
                value_4 = self.minimax_DLS(child, depth - 1, True, first_move, start_time)
